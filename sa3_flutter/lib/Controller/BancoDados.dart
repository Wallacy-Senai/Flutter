...
import 'package:sa3_flutter/Model/Usuario.dart';
...
class BancoDadosCrud {
 ...
 // Método para verificar se um usuário com o email especificado existe no banco de dados
  Future<bool> existeUsuario(String email) async {
    try {
      final Database db = await _getDatabase();
      final List<Map<String, dynamic>> maps = await db.query(
        Nome_Tabela,
        where: 'email = ?', // Condição apenas no email
        whereArgs: [email],
      );

      // Se a lista de resultados não estiver vazia, significa que um usuário com esse email já existe
